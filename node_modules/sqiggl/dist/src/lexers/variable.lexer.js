    ...
    VariableLexer.prototype.invoke = function (input) {
        ...
        while (idx < input.length) {
            switch (input.charAt(idx)) {
                ...
                case '[':
                    if (currentType === dsl_1.DSLVariableType.key)
                        throw new Error("SQiggLLexerError: Invalid character found in variable key '[' in '" + original + "'.");
                    if (!inString) {
                        if (idx !== startIdx)
                            throw new Error("SQiggLLexerError: Arrays in variables cannot be nested. At '" + original + "'.");
                ...
                case ']':
                    ...
                    if (!inString) {
                        if (idx !== input.length - 1)
                            throw new Error("SQiggLLexerError: Variables that define arrays must not include other values. At '" + original + "'.");
                        input = input.slice(0, idx) + input.slice(idx + 1);
                    }
                    console.log(input);
                    idx++;
